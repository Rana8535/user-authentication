...
const Login = () => {
  ...
  return (
    <div className='flex flex-col items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-blue-200 to bg-purple-400'>
      ...
      <div className='bg-slate-900 p-10 rounded-b-lg shadow-lg w-full sm:w-96 text-indigo-300 text-sm'>
        ...
        <form onSubmit={onsubmithandler}>
          {state ==="Sign Up" && (
            ...
            <p onClick={()=>navigate('/Resetpassword')} className='mb-4 text-indigo-500 cursor-pointer hover:text-indigo-200'>Forgot Password?</p>
            <button className='w-full py-2.5 rounded-full bg-gradient-to-r from-indigo-500 to-indigo-900 text-white font-medium'>{state}</button>
        </form>
        {state === "Sign Up"?(<p  className='text-gray-400 text-center text-xs mt-4'>Aready have an acoount?{" "}
          <span onClick={()=>setstate("Login")} className='text-blue-400 cursor-pointer underline'>Login here</span>
        </p>)
        :(
          <p  className='text-gray-400 text-center text-xs mt-4'>Don't have an acoount?{" "}
